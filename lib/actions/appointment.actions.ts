...
export const getAppointmentsByUser = async (userId: string) => {
    try {
        ...
        // return appointments.documents.map(doc => parseStringify(doc));
...
export const getFile = async (id: any) => {
    try {
        ...
        // console.log("apps", URL.createObjectURL(blob))
        // const imageUrl = URL.createObjectURL(blob);
        console.log("filesssss", image);
        return image;
    } catch (error) {
        console.error(
            "An error occurred while retrieving appointments:",
...
export const getFileUrl = async (id: any): Promise<any> => {
    try {
        const url = `${ENDPOINT}/storage/buckets/${DOCTOR_BUCKET_ID}/files/${id}/view?project=${PROJECT_ID}&mmode=admin`
